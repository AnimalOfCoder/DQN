...
# allocation from that base station only, 
# it is used only for checking if the resource is enouph for all users. 
# if res=0 is returned the slice is accepted, if res is non zero it will be ID of the slice 
...
def run():
    ...
    for T in range(0, 100000):
        ...
        s1r, s1Q, s1ar, P1, du, ru = Slice1.initSlice(max_users)# generating random users and and creating the network for slice1
        RL1.learn()
        s2r, s2Q, s2ar, P2, rtu, rsu = Slice2.initSlice(max_users)
        RL2.learn()
        #checking feasile users
        [S1,V,accept,S2, V2,accept2]=initial_VR2(s1r, s1ar + 1 / s1Q, s2r, s2Q)# filter users that cant be satisfied
        ...
        if S1 > 0 and S2 > 0: # removing the users that are not in range
            ...
            for bp2 in range(0, B):
                P1[0,bp2]=Puu1[bp2] / sum(Puu1)# priority or importance of the base stations to slice1
            
            Reservation.UpdateUnusedResource(S1,np.zeros((1,B)),Puu1)# minimim resource requirement os the slice1 on the base stations
            ...
            P2=np.zeros((1,B))
            for bp2 in range(0, B):
                P2[0,bp2]=Puu2[bp2] / sum(Puu2)# priority or importance of the base stations to slice2
            
            Reservation.UpdateUnusedResource(S2, np.zeros((1,B)),Puu2)#  minimim resource requirement os the slice2 on the base stations
            ...
            for ii in range(0, 50):
                ...
                _, _, AL1, RE1 = Reservation.sendCurrAllocAndReservOfSlice(1)# request allocation and reservation of slice 1 from all base stations
                _, _, AL2, RE2 = Reservation.sendCurrAllocAndReservOfSlice(2)#............slice2....
                ...
                if ii > 1: # if the iteration is not initial calculate the reward functions and call the DQN for trdaining, and increament the number of users at an interval of three iterations
                    ...
                    RL1.store_transition(cs1,a1,rw1,trn) # send the tuple to reply memory of
                    ...
                    RL2.store_transition(cs2,a2,rw2,trn)#  send the tuple to reply memory of
        ...
        Slice1.removeSliceFromResourceTable(S1)# remove slice 1 from resource table
        Slice2.removeSliceFromResourceTable(S2)# remove slice2 from resource table   
